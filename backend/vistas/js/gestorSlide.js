...
/*Creamos un slide por defecto y lo almacenamos en la Base de datos*/
...
$('.todo-list').sortable({
    ...
    stop: function(event){
        ...
        //recorremos todos los slide que tenga la clase .itemSlide
        for(var i = 0; i < itemSlide.length; i++){

            var datos = new FormData();
            datos.append("idSlide", event.target.children[i].id);  // lo que variara sera que id toma de primeras
            datos.append("orden", (i+1));  // el orden siempre sera 1, 2, 3 ...


            //cada vez que se cambia el orden se actualiza el Item por ID, con el nuevo orden
            $.ajax({
...
$(".nombreSlide").change(function(){
    ...
    $(guardarSlide[indiceSlide]).attr("nombreSlide", nombre);  // actualizamos el valor del btn en el atributo nombreSlide
